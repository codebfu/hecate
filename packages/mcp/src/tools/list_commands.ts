...
export function registerListCommandsTool(server: McpServer, client: HecateApiClient) {
  ...
  server.registerTool(
    ...
    async ({ machine_id, status, limit, offset }) => {
      ...
      return formatUntrustedToolResult(
        { commands: split.map((item) => item.metadata) },
        { commands: split.map((item) => item.untrustedOutput) },
      );
    },
  );
