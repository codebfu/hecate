...
export function registerRequestPermissionsTool(server, client) {
    const spec = findToolSpec("request_permissions");
    server.registerTool(spec.name, {
        description: "Submit a permanent permission change request for operator approval. Additive payload (grant assignments, not monolithic rules). One pending standard request per identity. Read hecate://skill/authz-model and hecate://skill/permission-requests first.",
        ...
        return formatUntrustedToolResult({ command: "permissions.request" }, response.result);
    });
    return spec;
}
export function registerReadGrantAssignmentsTool(server, client) {
    const spec = findToolSpec("read_grant_assignments");
    server.registerTool(spec.name, {
        description: "Read grant assignments for the current identity or another identity (requires admin.authz.assignments.read). See hecate://skill/authz-model.",
        inputSchema: z.object({
        ...
        return formatUntrustedToolResult({ command: "admin.authz.assignments.read" }, response.result);
    });
    return spec;
}
export function registerReadEffectiveRightsTool(server, client) {
    const spec = findToolSpec("read_effective_rights");
    server.registerTool(spec.name, {
        description: "Read the computed effective rights matrix for self or another identity (requires admin.authz.effective_rights.read). Prefer hecate://context/effective-rights for self.",
        inputSchema: z.object({
            identity_id: z.string().uuid().optional(),
        }),
        annotations: spec.annotations,
        ...
        return formatUntrustedToolResult({ command: "admin.authz.effective_rights.read" }, response.result);
    });
    return spec;
}
export function registerListPermissionRequestsTool(server, client) {
...
export function registerApprovePermissionRequestTool(server, client) {
    const spec = findToolSpec("approve_permission_request");
    server.registerTool(spec.name, {
        description: "Approve a pending permission request and apply requested rules permanently. Cannot approve own requests.",
        inputSchema: z.object({
        ...
        return formatUntrustedToolResult({ command: "admin.permissions.request.approve", request_id }, response.result);
    });
    return spec;
}
export function registerRejectPermissionRequestTool(server, client) {
    ...
    server.registerTool(spec.name, {
        ...
        return formatUntrustedToolResult({ command: "admin.permissions.request.reject", request_id }, response.result);
    });
    return spec;
}
export function registerListAuditEventsTool(server, client) {
...
export function registerListActionQueueTool(server, client) {
...
export function registerApproveQueueCommandTool(server, client) {
    const spec = findToolSpec("approve_queue_command");
    server.registerTool(spec.name, {
        description: "Approve a pending_approval command into the queue. Cannot approve commands enqueued by the current identity.",
        inputSchema: z.object({
        ...
        return formatUntrustedToolResult({ command: "admin.queue.approve", command_id }, response.result);
    });
    return spec;
}
export function registerCancelQueueCommandTool(server, client) {
    const spec = findToolSpec("cancel_queue_command");
    server.registerTool(spec.name, {
        description: "Cancel a pending_approval, queued, dispatched, or running command in the fleet action queue.",
        inputSchema: z.object({
            command_id: z.string().uuid(),
        }),
        annotations: spec.annotations,
        ...
        return formatUntrustedToolResult({ command: "admin.queue.cancel", command_id }, response.result);
    });
    return spec;
}
/** AI-authored reason / requested_changes must not drive the model as instructions. */
export function splitPermissionRequests(result) {
...
export function splitActionQueue(result) {
    if (!result || typeof result !== "object") {
        return { metadata: result, untrustedOutput: null };
    }
    const page = result;
...
export function splitAuditList(result) {
    if (!result || typeof result !== "object") {
        return { metadata: result, untrustedOutput: null };
    }
    const page = result;
...
const repoToolDefinitions = [
    ...
    },
    {
        ...
        description: "Update a feature repository source public key or priority. Official source URL is read-only.",
        inputSchema: z.object({
            id: z.string().min(1),
    ...
    },
    {
        ...
        description: "Remove a feature repository source. The official source cannot be removed. Installed features prevent removal.",
        inputSchema: z.object({ id: z.string().min(1) }),
    },
    {
    ...
    },
    {
        ...
        description: "Upgrade all installed features that track latest to the newest published version. Pinned features are skipped.",
        inputSchema: z.object({}),
    },
    {
...
const authzToolDefinitions = [
    {
        ...
        description: "List the aggregated authz catalog (fleet scopes, capability profiles, access grants). Requires admin.authz.catalog.",
        inputSchema: z.object({}),
    },
    {
    ...
    },
    {
        ...
        description: "Add or update grant assignments on a target identity (immediate effect). Requires admin.authz.assignments.add. See hecate://rule/authz-admin.",
        inputSchema: z.object({
            identity_id: z.string().uuid().optional(),
    ...
    },
    {
        ...
        description: "Remove grant assignments from a target identity (immediate effect). Cannot target self. Requires admin.authz.assignments.remove.",
        inputSchema: z.object({
            identity_id: z.string().uuid().optional(),
...
export function registerAuthzTools(server, client) {
    return authzToolDefinitions.map((definition) => {
        ...
        server.registerTool(spec.name, {
            ...
            const response = await client.executeAdminCommand(definition.command, params);
            return formatUntrustedToolResult({ command: definition.command }, response.result);
        });
        return spec;
    });
}
export function registerRepoTools(server, client) {
...
//# sourceMappingURL=admin_tools.js.map