...
export const DYNAMIC_PERMISSIONS_RESOURCE = {
    ...
    description: "Live identity, resolved grant assignments, effective summary, capabilities, and admin command allowlist.",
    mimeType: "application/json",
};
export const DYNAMIC_AUTHZ_CATALOG_RESOURCE = {
    uri: "hecate://context/authz-catalog",
    name: "Self-Service Authz Catalog",
    description: "S12-filtered catalog of assignable fleet scopes, capability profiles, and access grants for the authenticated identity.",
    mimeType: "application/json",
};
...
export function registerResources(server, client) {
    ...
    server.registerResource(DYNAMIC_PERMISSIONS_RESOURCE.name, DYNAMIC_PERMISSIONS_RESOURCE.uri, {
    ...
    server.registerResource(DYNAMIC_AUTHZ_CATALOG_RESOURCE.name, DYNAMIC_AUTHZ_CATALOG_RESOURCE.uri, {
    ...
    server.registerResource(DYNAMIC_EFFECTIVE_RIGHTS_RESOURCE.name, DYNAMIC_EFFECTIVE_RIGHTS_RESOURCE.uri, {
...
export async function readStaticResource(relativePath) {
    const fullPath = path.join(RESOURCES_ROOT, relativePath);
    return readFile(fullPath, "utf8");
}
export function resolveResourcePath(relativePath) {
